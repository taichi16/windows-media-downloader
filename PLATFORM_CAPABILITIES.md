...
MMOV 只把 `hk.mmov.io` page 經 Rust adapter 解析出的 verified HLS target 交給固定 yt-dlp；probe 必須回報 extractor=`generic`、protocol=`m3u8_native`。不接受前端或使用者直接提供 CDN URL，也不宣稱 yt-dlp 後續網路連線具有完整 sandbox。本次結果只適用於 2026-09-07 的單一使用者授權公開來源。
