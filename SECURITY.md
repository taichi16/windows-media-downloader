...
3. yt-dlp 使用 app resource 中固定 `yt-dlp.exe` 絕對路徑；固定加入 `--ignore-config`、`--no-plugin-dirs`、`--no-js-runtimes`、`--no-remote-components`、`--no-playlist`，並以 `--` 分隔 URL。前端不能提供任意參數、程序或輸出路徑。只有 MMOV 的 probe／下載固定加入官方 `--compat-options no-certifi`，改採 Windows 系統信任鏈但不關閉憑證驗證；其他平台維持既有信任來源。所有平台均禁止 `--no-check-certificates`。
...
9. 小鴨影音與歐樂影院 HLS 只接受固定媒體 host；驗證所有 manifest/segment URI 的 host 與公開 DNS、最多 3 層／5,000 URI，要求 `#EXTM3U` 與 leaf `#EXT-X-ENDLIST`，遇到 `EXT-X-KEY`／`SESSION-KEY` 即拒絕。MMOV 採獨立 exact host+port allowlist：`bfikuncdn.com:443`、`kkzycdn.com:65`、`b3.bdzybf22.com:443` 與 segment-only `tsb3.bdzybf22.com:443`；每個 effective endpoint 都依實際 port 解析 DNS 並驗證 response peer，禁止直接輸入 CDN URL；MMOV 另限制最多 32 個 distinct manifest、aggregate response body 8 MiB、整段 scan 60 秒 timeout，單一 response 1 MiB／20 秒 timeout。
...
13. MMOV 只接受 `hk.mmov.io` 的精確 VOD page path，數字 ID 有長度上限；source 不得有 query、fragment、userinfo、顯式 port 或 percent encoding。Rust 以固定 native-TLS client 解析唯一且有界的 `videoSrc`，不執行 JavaScript、不啟用 Cookie、登入、CAPTCHA、impersonation、geo 或 remote component。所有 master/variant/segment URI 必須是 HTTPS 且精確落在核准的 host/port 組合；`tsb3.bdzybf22.com:443` 僅可作為 manifest 內的 segment host，不能作為 page `videoSrc`。最多遞迴 3 層、5,000 URI、單 URI 4096 字元，要求 `#EXTM3U`、VOD `#EXT-X-ENDLIST`，只容許 `EXT-X-KEY:METHOD=NONE` 且無 URI，live-only tag、DASH、加密、malformed tag 與 master/leaf 混合均拒絕。MMOV probe 另要求 extractor=`generic`、protocol=`m3u8_native`。
    - 若站台瞬間回傳帶 query／fragment 的 MMOV HLS URI，adapter 會在 300 ms 後重新取得頁面並完整驗證一次；第二次仍不合規即拒絕。此機制不接受或保存 query/token，也不對 host、DNS、peer、VOD、KEY 或 URI 上限放寬。

Probe 的 `availability` 欄位可依 extractor 缺省（null）；若有值則只接受 `public`／`unlisted`，其他非空狀態（包含 `private`、`premium_only`、`subscriber_only`、`needs_auth`）一律拒絕。`has_drm` 取格式層明確標記；只要出現 `true` 或 `maybe` 即拒絕，缺省欄位不被誤判為 DRM。另讀取 `is_live`、`live_status`、`playlist_id` 與 `playlist_count`；直播、排程、已結束直播、非 `not_live` lifecycle 或 playlist marker 一律拒絕。Facebook probe 必須回報 `facebook` extractor；MMOV probe 必須回報 `generic` 與 `m3u8_native`，否則拒絕。這些 probe 閘門與 MMOV page/HLS adapter 會在每筆實際下載前重新執行。

## 本機歷史資料

SQLite 檔案固定在 Tauri `app_local_data_dir()/download-history.sqlite3`，schema 透過 transaction 由 `PRAGMA user_version=0` 遷移至 1；遇到較新版本不會刪除或覆寫。來源先通過既有 URL 正規化；一般平台移除全部 query 與 fragment，只保留允許的 scheme/host/必要安全 path。YouTube／YouTube Music 的 watch URL 僅保留安全 `v`，`youtu.be` 僅保留第一個安全短網址 ID；Facebook 僅保留 `/reel/<ID>` 或 `/頁面ID/videos/<影片ID>`，query（包含 fbcdn 簽章與 token）一律在來源驗證階段拒絕，歷史不保存。MMOV 僅保存 canonical `hk.mmov.io` page URL，絕不保存 CDN host、port 65、`videoSrc` 或 manifest URI。標題與檔名以 UTF-8 字元數截長，錯誤只映射到固定 enum-like code 與固定摘要。歷史不是 resume store，也不是 log。
...
平台 adapter 自己的 HTTP client 禁止 redirect，並對實際 peer 與所有 HLS URI host/effective port 做檢查；MMOV 也要求 response peer 屬於該次精確 `(host, port)` 的公開 DNS 集合。但目前沒有獨立 HTTP proxy 可攔截 yt-dlp/FFmpeg 下載 fragment 時的每一次 redirect，也沒有把所有後續 DNS 查詢鎖在啟動前結果的 OS 網路沙箱。因此仍不能宣稱完整防止 fragment redirect／DNS rebinding，也不能宣稱完整執行期網路 sandbox。若要提升保證，需在後續版本加入可驗證的 downloader network boundary，並重新進行威脅建模。
