...
9. 小鴨影音與歐樂影院 HLS 只接受固定媒體 host；驗證所有 manifest/segment URI 的 host 與公開 DNS、最多 3 層／5,000 URI，要求 `#EXTM3U` 與 leaf `#EXT-X-ENDLIST`，遇到 `EXT-X-KEY`／`SESSION-KEY` 即拒絕。MMOV 採獨立 exact host+port allowlist：`bfikuncdn.com:443`、`kkzycdn.com:65`；每個 effective endpoint 都依實際 port 解析 DNS 並驗證 response peer，禁止直接輸入 CDN URL；MMOV 另限制最多 32 個 distinct manifest、aggregate response body 8 MiB、整段 scan 60 秒 timeout，單一 response 1 MiB／20 秒 timeout。
